 ...
 */
export default function MatchView({ engine, completeFooter, onSaveForLater, savingForLater }) {
  ...
  if (status === 'complete') {
    return <MatchSummary state={state} footer={completeFooter} />;
  }
...
function InningsBreak({ state, narrative, onContinue, onSaveForLater, savingForLater }) {
  ...
  return (
    <div className="space-y-4 animate-pop-in">
      {onSaveForLater && <SaveForLaterBar onSave={onSaveForLater} saving={savingForLater} />}
      <div className="glass-strong relative overflow-hidden p-6 text-center">
        ...
        <div className="relative mt-4 rounded-xl border border-alert/30 bg-alert/10 px-4 py-3">
          ...
          <p className="scoreboard text-3xl font-extrabold text-alert">{state.target}</p>
          <p className="text-xs text-slate-400">to win in {state.config.totalOvers} overs</p>
        </div>
      </div>

      <div className="glass p-4 text-center text-sm text-slate-300">{narrative}</div>
