    ...
    <form onSubmit={submit} className="space-y-3">
          ...
          {BATTING_HANDS.map((h) => {
            ...
            return (
              <button
                ...
                className={`btn-press rounded-lg px-2 py-2 text-sm font-semibold transition ${
                  active ? 'bg-neon/[0.12] text-neon ring-1 ring-neon/40' : 'text-slate-400 hover:text-slate-200 hover:bg-white/5'
                }`}
              >
                {h.label}
              </button>
      ...
      <div>
        ...
        <select
          ...
          className="w-full appearance-none rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-base text-slate-100 outline-none transition focus:border-neon/50 focus:ring-2 focus:ring-neon/20"
        >
      ...
      <button
        ...
        className="btn-press flex w-full items-center justify-center gap-2 rounded-2xl bg-neon py-3.5 text-base font-bold text-midnight shadow-glow-green disabled:opacity-40"
      >
